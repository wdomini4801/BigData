...
def create_hdfs_directory_command(container_name, hdfs_target_dir):
  ...
  create_dir_command = f"docker exec {container_name} hdfs dfs -mkdir -p {hdfs_target_dir}"
...
def copy_files_to_docker_command(container_name, local_source_dir, staging_dir_in_container):
  ...
  copy_files_command = f"docker cp {local_source_dir}/. {container_name}:{staging_dir_in_container}/"
...
def upload_to_hdfs_command(container_name, hdfs_target_dir, staging_dir_in_container):
  ...
  upload_command = f'docker exec {container_name} sh -c "hdfs dfs -put -f {staging_dir_in_container}/* {hdfs_target_dir}/"'
